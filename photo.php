...
class Photo {
    ...
    function getDimensionRatio() {
     ...
     public function doResize($max_dimension,$directory,$filenameout) {
        
        $errors = array();
        $this->setDimensions($max_dimension);
        $success = $this->resize($directory,$filenameout);
        if(!$success) { $errors[] = 'Sorry! I couldn\'t resize your photo.  Please try again later.'; }
        return $errors;    
    }
    
	public function doResizeHeight($max_dimension,$directory,$filenameout) {
        
        $errors = array();
        $this->setDimensionsHeight($max_dimension);
        $success = $this->resize($directory,$filenameout);
        if(!$success) { $errors[] = 'Sorry! I couldn\'t resize your photo.  Please try again later.'; }
        return $errors;    
    }
	
	public function doResizeCrop($resize_dimension_w, $resize_dimension_h ,$x,$y,$directory, $filenameout) {
        
		$errors = array();
        $success = $this->setSelectCropDimensions($resize_dimension_w, $resize_dimension_h, $x,$y);
        $success = $success && $this->resize($directory,$filenameout);
    ...
    public function doThumb($thumb_size,$directory,$filenameout) {
    ...
    public function doCenterCrop($x,$y,$directory='',$filenameout) {
    ...
    public function doFullCrop($x,$y,$width,$height,$directory='',$filenameout) {
	...
	private function setDimensionsHeight($max_dimension) {
		$this->dst_height = $max_dimension;
		$this->dst_width = ($this->orig_width*$this->dst_height)/$this->orig_height;
        $this->src_width = $this->orig_width;
        $this->src_height = $this->orig_height;
	...
	private function setSelectCropDimensions($resize_dimension_w, $resize_dimension_h, $x,$y) {
	   $this->src_x = $x/2;
	   $this->src_y = $y/2;
	   $this->src_width = $this->orig_height;
	   $this->src_height = $this->orig_height;
        ...
        $this->dst_width = $resize_dimension_w;
        $this->dst_height = $resize_dimension_h;
        $this->dst_x = 0;
        $this->dst_y = 0;
    ...
    private function resize($directory,$filenameout) {
        ...
        $filename = $directory.$filenameout;
		if($this->mime == 'image/png') {
			$success =   @imagepng($this->tmp,$filename,100);
